...
export default function AddService({ searchParams }: { searchParams: { message: string } }) {
  ...
  const createService = async (formData: FormData) => {
    ...
    if (Error()) {
      return redirect("/?message=Could not add service");
    }
  ...
  return (
    <div className="bg-cover col-start-1 col-span-6 row-start-3 row-span-3 mx-auto flex flex-col w-full px-8 sm:max-w-md justify-center gap-6">
      ...
      <form className="flex flex-col gap-4" action={createService}>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-md font-medium" htmlFor="strategy" aria-required>
            Strategy
          </label>
